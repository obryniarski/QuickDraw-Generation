...
class ResBlock(nn.Module):
    def __init__(self, in_filters, num_filters, slope=0.02):
...
class Discriminator(nn.Module):
    def __init__(self, imsize=28):
        ...
        self.linear = nn.Linear(1, imsize**2)
    ...
    def forward(self, input, label):
        ...
        up_class = self.linear(label).view(-1, 1, 28, 28)
        input = torch.cat([input, up_class], dim=1)
